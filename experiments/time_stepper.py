...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--solver", choices=["direct", "resnet", "euler"], default="euler"
    )
    ...
    parser.add_argument("--n_traj_train", default=1000)
    parser.add_argument("--n_traj_validate", default=10)
    parser.add_argument("--t_start_train", default=0.0)
    parser.add_argument("--t_end_train", type=float, default=0.002)
    parser.add_argument("--t_start_validate", default=0.0)
    parser.add_argument("--t_end_validate", type=float, default=4 * np.pi)
    parser.add_argument("--step_size_train", default=0.001)
    parser.add_argument("--step_size_validate", default=0.001)
    ...
    t_span_train = torch.arange(
        args.t_start_train, args.t_end_train, args.step_size_train
    )
    ...
    for _ in tqdm(range(args.n_epochs)):
        ...
        _, x_pred_train = odeint(
            lambda t, x: net(x), x0_train, t_span_train, solver=args.solver
        )
    ...
    _, x_pred_validate = odeint(
        lambda t, x: net(x), x0_validate, t_span_validate, solver=args.solver
    )

    _, x_pred_example = odeint(
        lambda t, x: net(x), x0_example, t_span_validate, solver=args.solver
    )
